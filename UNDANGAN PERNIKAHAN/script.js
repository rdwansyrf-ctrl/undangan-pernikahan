document.addEventListener("DOMContentLoaded", () => {
  ...
  // Generator Link WA Functionality
  const btnGenWA = document.getElementById("btnGenWA");
  if (btnGenWA) {
    btnGenWA.addEventListener("click", () => {
      const name = document.getElementById("genName").value.trim();
      const phone = document.getElementById("genPhone").value.trim();

      if (!name || !phone) {
        showToast("Harap isi nama dan nomor WA!");
        return;
      }

      const currentUrl = window.location.origin + window.location.pathname;
      const link = `${currentUrl}?to=${encodeURIComponent(name)}`;
      const text = `Halo ${name}, kami mengundang Anda ke acara pernikahan kami. Buka undangan melalui link berikut:\n\n${link}`;
